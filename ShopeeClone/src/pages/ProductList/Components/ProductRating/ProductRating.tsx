interface Props {
  rating: number
}

export default function ProductRating({ rating }: Props) {
  const handleRating = (_index: number) => {
    if (_index <= rating) {
      return '100%'
    }
    if (_index > rating && _index - rating < 1) {
      return `${rating - _index}%`
    }
    if (_index > rating && _index - rating > 1) {
      return '0%'
    }
  }

  return (
    <div className='flex items-center'>
      {Array(5)
        .fill(0)
        .map((_, index) => (
          <div className='relative' key={index}>
            <div className='absolute top-0 left-0 h-full overflow-hidden' style={{ width: handleRating(index + 1) }}>
              <svg
                enableBackground='new 0 0 15 15'
                viewBox='0 0 15 15'
                x='0'
                y='0'
                className='h-3 w-3 fill-yellow-400 text-yellow-300'
              >
                <polygon
                  points='7.5 .8 9.7 5.4 14.5 5.9 10.7 9.1 11.8 14.2 7.5 11.6 3.2 14.2 4.3 9.1 .5 5.9 5.3 5.4'
                  strokeLinecap='round'
                  strokeLinejoin='round'
                  strokeMiterlimit='10'
                ></polygon>
              </svg>
            </div>
            <svg
              enableBackground='new 0 0 15 15'
              viewBox='0 0 15 15'
              x='0'
              y='0 '
              className='h-3 w-3 fill-current text-gray-200'
            >
              <polygon
                points='7.5 .8 9.7 5.4 14.5 5.9 10.7 9.1 11.8 14.2 7.5 11.6 3.2 14.2 4.3 9.1 .5 5.9 5.3 5.4'
                strokeLinecap='round'
                strokeLinejoin='round'
                strokeMiterlimit='10'
              ></polygon>
            </svg>
          </div>
        ))}
    </div>
  )
}

// cách tính rating vd rating  = 3.4
// sao
// 1 < 3.4 => 100% tô
// 2 < 3.4 => 100% tô
// 3 < 3.4 => 100%
// 4 > 3.4 => 4-3.4 < 1 = 0.4 => 40%
// 5 > 3.4 => 5-3.4 > 1 = 0 => 0%
